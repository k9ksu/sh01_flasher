...
> This tool rewrites the firmware of a mains-powered heating appliance and,
> if you raise the HH trip, removes its only over-temperature protection.
> A mistake here can ruin the dryer, melt filament or the enclosure, or
> start a fire. It voids any warranty. You are responsible for verifying
> your wiring, choosing sane limits, and not running a modified dryer
> unattended until you've watched it behave. Use at your own risk; the
> author accepts no liability for damage or injury.
...
"auto" in the GUI or pass `--hh` to set it yourself. Raising the trip above
stock requires an explicit acknowledgement: typing a phrase in the GUI, or
`--i-accept-the-risk` on the CLI. Either way you have moved the safety limit
— see Hardware notes.
